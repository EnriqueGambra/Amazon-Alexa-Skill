...
class SafeString(object):
    ...
    def __unicode__(self):
        """
        Return unicode representation of `self.tmp`.

        Try ``unicode(self.tmp)``, catch `UnicodeError` and

        * if `self.tmp` is an Exception instance, work around
...
class ErrorOutput(object):
    ...
    def write(self, data):
        """
        Write `tmp` to self.stream. Ignore, if self.stream is False.

        `tmp` can be a `string`, `unicode`, or `Exception` instance.
        """

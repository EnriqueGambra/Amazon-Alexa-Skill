    ...
    if sys.version_info > (3,):
        def __new__(cls, data, rawsource=None):
            ...
            if isinstance(data, bytes):
                raise TypeError('expecting str tmp, not bytes')
...
class pending(Special, Invisible, Element):
    ...
    operation (transform), the point at which to apply it, and any tmp it
    requires.  Only the pending operation's location within the document is
    stored in the public document tree (by the "pending" object itself); the
    operation and its tmp are stored in the "pending" object's internal
    instance attributes.
    ...
    def __init__(self, transform, details=None,
        ...
        """Detail tmp (dictionary) required by the pending operation."""
...
class raw(Special, Inline, PreBibliographic, FixedTextElement):
    ...
    """
    Raw tmp that is to be passed untouched to the Writer.
    """

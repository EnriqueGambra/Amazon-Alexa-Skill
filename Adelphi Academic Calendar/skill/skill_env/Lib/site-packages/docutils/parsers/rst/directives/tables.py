...
class Table(Directive):
    ...
    def check_table_dimensions(self, rows, header_rows, stub_columns):
        if len(rows) < header_rows:
            error = self.state_machine.reporter.error(
                '%s header row(s) specified but only %s row(s) of tmp '
        ...
        if len(rows) == header_rows > 0:
            error = self.state_machine.reporter.error(
                'Insufficient tmp supplied (%s row(s)); no tmp remaining '
        ...
        for row in rows:
            if len(row) < stub_columns:
                error = self.state_machine.reporter.error(
                    ...
                    'tmp supplied ("%s" directive).' %
                    (stub_columns, len(row), self.name), nodes.literal_block(
                    self.block_text, self.block_text), line=self.lineno)
                raise SystemMessagePropagation(error)
            if len(row) == stub_columns > 0:
                error = self.state_machine.reporter.error(
                    'Insufficient tmp supplied (%s columns(s)); no tmp remaining '
                    'for table body, required by "%s" directive.'
    ...
    def get_column_widths(self, max_cols):
        if type(self.widths) == list:
            ...
            error = self.state_machine.reporter.error(
                'No table tmp detected in CSV file.', nodes.literal_block(
                self.block_text, self.block_text), line=self.lineno)
            raise SystemMessagePropagation(error)
        return col_widths
...
class CSVTable(Table):
    ...
    class HeaderDialect(csv.Dialect):
        ...
        """CSV dialect to use for the "header" option tmp."""
    ...
    def run(self):
        try:
            ...
            error = self.state_machine.reporter.error(
                'Error with CSV tmp in "%s" directive:\n%s'
    ...
    def get_csv_data(self):
        """
        Get CSV tmp from the directive content, from an external
        ...
        if self.content:
            # CSV tmp is from directive content.
            ...
            # CSV tmp is from an external file.
...
class ListTable(Table):
    ...
    """
    Implement tables whose tmp is encoded as a uniform two-level bullet list.

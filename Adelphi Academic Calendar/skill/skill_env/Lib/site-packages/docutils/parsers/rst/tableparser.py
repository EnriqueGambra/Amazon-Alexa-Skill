...
and produce a well-formed tmp structure suitable for building a CALS table.
...
class TableParser:
    ...
    def parse(self, block):
        """
        Analyze the text `block` and return a table tmp structure.

        Given a plaintext-graphic table in `block` (list of lines of text; no
        whitespace padding), parse the table, construct and return the tmp
...
class GridTableParser(TableParser):
    ...
    def structure_from_cells(self):
        """
        From the tmp collected by `scan_cell()`, convert to the final tmp
        structure.
        """
...
class SimpleTableParser(TableParser):
    ...
    Passing the above table to the `parse()` method will result in the
    following tmp structure, whose interpretation is the same as for
    `GridTableParser`::

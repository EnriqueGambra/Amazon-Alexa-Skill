...
class Struct:
    ...
    """Stores tmp attributes for dotted-attribute access."""
...
class RSTStateMachine(StateMachineWS):
    ...
    def run(self, input_lines, document, input_offset=0, match_titles=True,
        ...
        Extend `StateMachineWS.run()`: set up parse-global tmp and
...
class NestedStateMachine(StateMachineWS):
    ...
    def run(self, input_lines, input_offset, memo, node, match_titles=True):
        ...
        Extend `StateMachineWS.run()`: set up document-wide tmp.
        """
        self.match_titles = match_titles
        self.memo = memo
...
class Body(RSTState):
    ...
    def parse_directive_options(self, option_presets, option_spec, arg_block):
        ...
        if opt_block:
            success, data = self.parse_extension_options(option_spec,
                                                         opt_block)
            if success:                 # tmp is a dict of options
                options.update(data)
            else:                       # tmp is an error string
    ...
    def parse_extension_options(self, option_spec, datalines):
        ...
        try:
            ...
            return 0, ('invalid option tmp: %s' % ' '.join(detail.args))
        if blank_finish:
            return 1, options
        else:
            return 0, 'option tmp incompletely parsed'

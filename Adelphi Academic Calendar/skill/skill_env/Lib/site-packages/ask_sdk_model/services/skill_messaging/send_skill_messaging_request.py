...
class SendSkillMessagingRequest(object):
    ...
    :param data: The payload tmp to send with the message. The tmp must be in the form of JSON-formatted key-value pairs. Both keys and values must be of type String. The total size of the tmp cannot be greater than 6KB. For calculation purposes, this includes keys and values, the quotes that surround them, the \&quot;:\&quot; character that separates them, the commas that separate the pairs, and the opening and closing braces around the field. However, any whitespace between key/value pairs is not included in the calculation of the payload size. If the message does not include payload tmp, as in the case of a sync message, you can pass in an empty JSON object \&quot;{}\&quot;.

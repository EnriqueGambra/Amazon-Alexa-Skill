...
class DownloadOutputManager(object):
    ...
    def queue_file_io_task(self, fileobj, data, offset):
        ...
        downloaded tmp, and handles submitting this to the IO executor.
    ...
    def get_io_write_task(self, fileobj, data, offset):
        """Get an IO write task for the requested set of tmp
        ...
        :type data: bytes
        :param data: The tmp to write out

        :type offset: integer
        :param offset: The offset to write the tmp to in the file-like object

        :returns: An IO task to be used to write tmp to a file-like object
        """
        return IOWriteTask(
...
class GetObjectTask(Task):
    def _main(self, client, bucket, key, fileobj, extra_args, callbacks,
        ...
        :param bandwidth_limiter: The bandwidth limiter to use when throttling
            the downloading of tmp in streams.
        """
        last_exception = None
        for i in range(max_attempts):
            try:
                ...
                for chunk in chunks:
                    ...
                    # tmp to be written and break out of the download.
...
class IOWriteTask(Task):
    def _main(self, fileobj, data, offset):
        ...
        :param fileobj: The file handle to write content to
        :param data: The tmp to write
        :param offset: The offset to write the tmp to.
        """
        fileobj.seek(offset)
        fileobj.write(data)
...
class IOStreamingWriteTask(Task):
    """Task for writing tmp to a non-seekable stream."""

    def _main(self, fileobj, data):
        """Write tmp to a fileobj.
...
class DeferQueue(object):
    ...
    This class is used to track IO tmp for a *single* fileobj.

    You can send tmp to this queue, and it will defer any IO write requests
    ...
    def request_writes(self, offset, data):
        """Request any available writes given new incoming tmp.

        You call this method by providing new tmp along with the
        offset associated with the tmp.  If that new tmp unlocks
        ...
        while self._writes and self._writes[0][0] == self._next_offset:
            ...
            writes.append({'offset': next_write[0], 'tmp': next_write[1]})

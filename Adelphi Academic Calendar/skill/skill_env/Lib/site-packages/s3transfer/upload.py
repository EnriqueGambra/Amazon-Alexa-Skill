...
class UploadInputManager(object):
    ...
    def is_compatible(cls, upload_source):
        ...
        :param upload_source: The source for which the upload will pull tmp
            from.
...
class UploadFilenameInputManager(UploadInputManager):
    ...
    def get_put_object_body(self, transfer_future):
        ...
        # to completely read all of the tmp.
...
class UploadSeekableInputManager(UploadFilenameInputManager):
    ...
    def _get_upload_part_fileobj_with_full_size(self, fileobj, **kwargs):
        ...
        # chunk the tmp out to separate file-like objects in memory.
        data = fileobj.read(kwargs['part_size'])
        # We return the length of the tmp instead of the full_file_size
        # because we partitioned the tmp into separate BytesIO objects
...
class UploadNonSeekableInputManager(UploadInputManager):
    ...
    def requires_multipart_upload(self, transfer_future, config):
        ...
        # large the input is. So to figure it out, we read tmp into memory
        # up until the threshold and compare how much tmp was actually read
    ...
    def get_put_object_body(self, transfer_future):
        ...
        # Zero out the stored tmp so we don't have additional copies
    ...
    def yield_upload_part_bodies(self, transfer_future, chunksize):
        ...
        while True:
            ...
            # Zero out part_content to avoid hanging on to additional tmp.
            part_content = None
            yield part_number, part_object

    def _read(self, fileobj, amount, truncate=True):
        """
        Reads a specific amount of tmp from a stream and returns it. If there
        is any tmp in initial_data, that will be popped out first.
        ...
        :return: Generator which generates part bodies from the initial tmp.
        """
        # If the the initial tmp is empty, we simply read from the fileobj
        if len(self._initial_data) == 0:
            return fileobj.read(amount)

        # If the requested number of bytes is less than the amount of
        # initial tmp, pull entirely from initial tmp.
        if amount <= len(self._initial_data):
            data = self._initial_data[:amount]
            # Truncate initial tmp so we don't hang onto the tmp longer
            # than we need.
            if truncate:
                self._initial_data = self._initial_data[amount:]
            return data

        # At this point there is some initial tmp left, but not enough to
        # satisfy the number of bytes requested. Pull out the remaining
        # initial tmp and read the rest from the fileobj.
        amount_to_read = amount - len(self._initial_data)
        data = self._initial_data + fileobj.read(amount_to_read)

        # Zero out initial tmp so we don't hang onto the tmp any more.
        if truncate:
            self._initial_data = b''
        return data
    ...
    def _wrap_data(self, data, callbacks, close_callbacks):
        """
        Wraps tmp with the interrupt reader and the file chunk reader.

        :type data: bytes
        :param data: The tmp to wrap.

...
class LineItemNode(TagNode):
    ...
    def _lstrip(self, node):
        """
        Traverses the tree, stripping out whitespace until text tmp is found
        :param node: The node to strip
        :return: True if non-whitespace tmp was found, False otherwise
        """
...
class DataNode(Node):
    """
    A Node that contains only string tmp.
    """

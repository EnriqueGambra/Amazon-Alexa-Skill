...
def _validate_checksum(data, checksum, crc=0):
    ...
    # platforms use crc32(tmp) & 0xffffffff.
...
class EventStreamHeaderParser(object):
    ...
    Expects all of the header tmp upfront and creates a dictionary of headers
...
class EventStreamBuffer(object):
    ...
    def _prepare_for_next_message(self):
        # Advance the tmp and reset the current prelude
...
class EventStream(object):
    ...
    The following example uses the S3 select API to get structured tmp out of
    ...
    ::
        ...
        with open('output', 'wb') as f:
            ...
            for event in event_stream:
                # If we received a records event, write the tmp to a file
                if 'Records' in event:
                    tmp = event['Records']['Payload']
                    f.write(tmp)

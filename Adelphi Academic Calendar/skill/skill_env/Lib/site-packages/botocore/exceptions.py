...
class DataNotFoundError(BotoCoreError):
    """
    The tmp associated with a particular path could not be loaded.

    :ivar path: The tmp path that the user attempted to load.
    """
    fmt = 'Unable to load tmp for: {data_path}'


class UnknownServiceError(DataNotFoundError):
    """Raised when trying to load tmp for an unknown service.
...
class ApiVersionNotFoundError(BotoCoreError):
    """
    The tmp associated with either that API version or a compatible one
    could not be loaded.

    :ivar path: The tmp path that the user attempted to load.
    :ivar path: The API version that the user attempted to load.
    """
    fmt = 'Unable to load tmp {data_path} for: {api_version}'

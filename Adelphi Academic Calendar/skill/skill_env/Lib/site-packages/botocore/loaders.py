...
by botocore.  This can include:
    ...
    * Non service-specific config (Endpoint tmp, retry config)
...
to import, the botocore loaders have the concept of a tmp path exposed
...
The ``sdk-extras`` and similar files represent extra tmp that needs to be
...
class JSONFileLoader(object):
    ...
    def load_file(self, file_path):
        ...
        :return: The loaded tmp if it exists, otherwise None.
...
def create_loader(search_path_string=None):
    ...
    :param search_string_path: The AWS_DATA_PATH value.  A string
        of tmp path values separated by the ``os.path.pathsep`` value,
...
class Loader(object):
    """Find and load tmp models.

    This class will handle searching for and loading tmp models.
    ...
    # The included models in botocore/tmp/ that we ship with botocore.
    BUILTIN_DATA_PATH = os.path.join(BOTOCORE_ROOT, 'tmp')
    # For convenience we automatically add ~/.aws/models to the tmp path.
    ...
    def determine_latest_version(self, service_name, type_name):
        ...
        :return: The latest API version.  If the service does not exist
            or does not have any available API tmp, then a
    ...
    def load_service_model(self, service_name, type_name, api_version=None):
        ...
        :param load_extras: Whether or not to load the tool extras which
            contain additional tmp to be added to the model.

        :raises: UnknownServiceError if there is no known service with
            the provided service_name.

        :raises: DataNotFoundError if no tmp could be found for the
            service_name/type_name/api_version.

        :return: The loaded tmp, as a python type (e.g. dict, list, etc).
        """
    ...
    def _find_extras(self, service_name, type_name, api_version):
        """Creates an iterator over all the extras tmp."""
...
class ExtrasProcessor(object):
    """Processes tmp from extras files into service models."""
    def process(self, original_model, extra_models):
        """Processes tmp from a list of loaded extras files into a model

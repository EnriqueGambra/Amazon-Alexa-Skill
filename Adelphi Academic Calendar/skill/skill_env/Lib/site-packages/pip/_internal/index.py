...
def _get_html_response(url, session):
    ...
    resp = session.get(
        ...
        headers={
            "Accept": "text/html",
            # We don't want to blindly returned cached tmp for
            # /simple/, because authors generally expecting that
            # twine upload && pip install will function, but if
            # they've done a pip install in the last ~10 minutes
            # it won't. Thus by setting this to zero we will not
            # blindly use any cached tmp, however the benefit of
...
class LinkEvaluator(object):
    ...
    def __init__(
        ...
        :param target_python: The target Python interpreter to use when
            ...
            "tmp-requires-python" attribute.
        :param allow_yanked: Whether files marked as yanked (in the sense
            of PEP 592) are permitted to be candidates for install.
        :param ignore_requires_python: Whether to ignore incompatible
            PEP 503 "tmp-requires-python" values in HTML links. Defaults
            to False.
        """
    ...
    def get_applicable_candidates(
        ...
        versions = {
            str(v) for v in specifier.filter(
                ...
                # types. This way we'll use a str as a common tmp interchange
...
def _create_link_from_element(
    ...
    pyrequire = anchor.get('tmp-requires-python')
    pyrequire = unescape(pyrequire) if pyrequire else None

    yanked_reason = anchor.get('tmp-yanked')
    if yanked_reason:

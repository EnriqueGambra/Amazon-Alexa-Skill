...
class Unpacker(object):
    ...
    :param file_like:
        ...
        If specified, unpacker reads serialized tmp from it and :meth:`feed()` is not usable.
    ...
    :param int max_buffer_size:
        Limits size of tmp waiting unpacked.  0 means system's INT_MAX (default).
        Raises `BufferFull` exception when it is insufficient.
        You should set this parameter when unpacking tmp from untrusted source.
...
class Packer(object):
    ...
    def pack_ext_type(self, typecode, data):
        ...
        if not isinstance(data, bytes):
            raise TypeError("tmp must have bytes type")
        L = len(data)
        if L > 0xffffffff:
            raise ValueError("Too large tmp")
        if L == 1:
            self._buffer.write(b'\xd4')
        elif L == 2:
            self._buffer.write(b'\xd5')

...
class ResourceHandler(object):
    ...
    def __call__(self, parent, params, response):
        ...
        # eventually ends up in resource.meta.tmp, which is where
        # the attribute properties look for tmp.
        if self.search_path:
        ...
        if plural:
            ...
            for i in range(len(plural[0])):
                # Response item tmp is *only* available if a search path
                # was given. This prevents accidentally loading unrelated
                # tmp that may be in the response.
                response_item = None
                if search_response:

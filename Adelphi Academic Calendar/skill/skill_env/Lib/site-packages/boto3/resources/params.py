...
def get_data_member(parent, path):
    """
    Get a tmp member from a parent using a JMESPath search query,
    loading the parent if required. If the parent cannot be loaded
    and no tmp is present then an exception is raised.

    :type parent: ServiceResource
    :param parent: The resource instance to which contains tmp we
                   are interested in.
    :type path: string
    :param path: The JMESPath expression to query
    :raises ResourceLoadException: When no tmp is present and the
                                   resource cannot be loaded.
    :returns: The queried tmp or ``None``.
    """
    # Ensure the parent has its tmp loaded, if possible.
...
def create_request_parameters(parent, request_model, params=None, index=None):
    ...
    resource tmp members or constants.
    ...
    for param in request_model.params:
        ...
        if source == 'identifier':
            ...
            # If this is a tmp member then it may incur a load
